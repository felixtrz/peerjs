import { EventEmitterWithError, type EventsWithError } from "../utils/error";
...
> {
	...
	private _getOrCreateChannel(
		...
		const connection = this.provider._createDataConnection(this, options);
		if (connection) {
	...
	close(): void {
		...
		if (this.provider) {
			this.provider._removeNode(this.peer);
		}
