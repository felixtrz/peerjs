...
class SerializationPage {
	...
	async open(testFile, serialization) {
		await browser.switchWindow("Alice");
		await browser.url(
			`/e2e/datachannel/serialization/serialization.html?testfile=${testFile}&serialization=${serialization}&key=${BYPASS_WAF}#Alice`,
		);
		await this.connectBtn.waitForEnabled();

		await browser.switchWindow("Bob");
		await browser.url(
			`/e2e/datachannel/serialization/serialization.html?testfile=${testFile}&key=${BYPASS_WAF}#Bob`,
		);
		await this.connectBtn.waitForEnabled();
	}
	async init() {
